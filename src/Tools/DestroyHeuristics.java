		...
		for(int i =0; i < nRemove; i++) {
			...
			for(ContractGroup group: solution.getNewSchedule().keySet()) {
				if(group.getNr() == contGroup) {
				 ...
				 if(instance.getFromDutyNrToDuty().containsKey(solution.getNewSchedule().get(group).getSchedule().getSchedule()[dutyDay])){
					Request request = new Request(instance.getFromDutyNrToDuty().get(solution.getNewSchedule().get(group).getSchedule().getSchedule()[dutyDay]), group, dutyDay);	
					solution.removeRequest(request, solution, slots, dutyDay);
				 }
				 else if(instance.getFromRDutyNrToRDuty().containsKey(solution.getNewSchedule().get(group).getSchedule().getSchedule()[dutyDay])) {
					 Request request = new Request(instance.getFromRDutyNrToRDuty().get(solution.getNewSchedule().get(group).getSchedule().getSchedule()[dutyDay]), group, dutyDay);	
					solution.removeRequest(request, solution, slots, dutyDay);
				...
				for(int index: removalDuties.get(group)) {			   
					...
					if(solution.getNewSchedule().get(group).getWeeklyOvertime()[index] > 0) {
						...
						for(int w = index*7; w <= index*7+6; w++) {
							if(instance.getFromDutyNrToDuty().containsKey(solution.getNewSchedule().get(group).getSchedule().getSchedule()[w])) {
								Duty duty = instance.getFromDutyNrToDuty().get(solution.getNewSchedule().get(group).getSchedule().getSchedule()[w]);
							...
							else if(instance.getFromRDutyNrToRDuty().containsKey(solution.getNewSchedule().get(group).getSchedule().getSchedule()[w])){
								ReserveDutyType reserveDuty = instance.getFromRDutyNrToRDuty().get(solution.getNewSchedule().get(group).getSchedule().getSchedule()[w]);
							...
							else if(solution.getNewSchedule().get(group).getSchedule().getSchedule()[index] == 1) {
