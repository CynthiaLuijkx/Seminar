...
public class DestroyHeuristics {
	...
	public void executeRandomOvertime(Solution solution, int nRemove, Random random, Instance instance) {
			...
			for(int index: removalDuties.get(group)) {			   
				...
				if(solution.getNewSchedule().get(group).getWeeklyOvertime()[index] > 0) {
					...
					for(int w = index*7; w <= index*7+6; w++) {
						if(instance.getFromDutyNrToDuty().containsKey(solution.getNewSchedule().get(group).getLSSchedule().getSchedule()[w])) {
							Duty duty = instance.getFromDutyNrToDuty().get(solution.getNewSchedule().get(group).getLSSchedule().getSchedule()[w]);
						...
						else if(instance.getFromRDutyNrToRDuty().containsKey(solution.getNewSchedule().get(group).getLSSchedule().getSchedule()[w])){
							ReserveDutyType reserveDuty = instance.getFromRDutyNrToRDuty().get(solution.getNewSchedule().get(group).getLSSchedule().getSchedule()[w]);
						...
						else if(solution.getNewSchedule().get(group).getLSSchedule().getSchedule()[index] == 1) {
