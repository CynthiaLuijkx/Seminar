...
public class Solution {
	...
	public void removeRequest(Request request, Solution solution, Set<TimeSlot> emptyTimeSlots, int dutyDay) {
		if(!requests.contains(request)) {
			...
			for(int k = 7*weekNumber; k < (7*weekNumber+6); k++) {
				if(instance.getFromDutyNrToDuty().containsKey(solution.getNewSchedule().get(request.getGroup()).getSchedule().getSchedule()[k])) {
					sum += instance.getFromDutyNrToDuty().get(solution.getNewSchedule().get(request.getGroup()).getSchedule().getSchedule()[k]).getPaidMin();
				}
				else if(instance.getFromRDutyNrToRDuty().containsKey(solution.getNewSchedule().get(request.getGroup()).getSchedule().getSchedule()[k])) {
					sum +=solution.getNewSchedule().get(request.getGroup()).getSchedule().getC().getAvgHoursPerDay()*60;
				}
				else if(solution.getNewSchedule().get(request.getGroup()).getSchedule().getSchedule()[k] == 1) {
					sum += solution.getNewSchedule().get(request.getGroup()).getSchedule().getC().getAvgHoursPerDay()*60;
				}
			}
			solution.getNewSchedule().get(request.getGroup()).getWeeklyOvertime()[weekNumber] = sum;
			solution.getNewSchedule().get(request.getGroup()).getSchedule().getSchedule()[dutyDay] = 2;
	...
	public void addRequest(Placement placement) {
		...
		currentSchedule.getSchedule().getSchedule()[placement.getTimeslot().getDay()] = placement.getRequest().getDutyNumber(); 
