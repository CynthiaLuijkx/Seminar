...
public class DirectedGraph<V,A>
{
	...
	public void removeArc(DirectedGraphArc<V, A> curArc) throws IllegalArgumentException {
		if (curArc == null) {
			...
			arcs.remove(curArc);
			outArcs.get(curArc.getFrom()).remove(curArc);
			inArcs.get(curArc.getTo()).remove(curArc);
		}
	}
	...
	public void removeNode(V node) throws IllegalArgumentException {
		if (node == null) {
			...
			for (DirectedGraphArc<V, A> out : outArcs.get(node)) {
				inArcs.get(out.getTo()).remove(out);
				arcs.remove(out);
			}
			inArcs.remove(node);
			outArcs.remove(node);
