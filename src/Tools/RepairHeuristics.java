...
public class RepairHeuristics {
	...
	public List<Placement> setPlacements(Request request, Solution solution, Set<ContractGroup> groups){
		...
		for(ContractGroup group: groups) {
			...
			LSschedule lsschedule = solution.getNewSchedule().get(group); 
			for(int i = request.getWeekday(); i < lsschedule.getSchedule().getSchedule().length; i+=7) {
				if(solution.getNewSchedule().get(group).getSchedule().getSchedule()[i] == 2) {
					double costOfPlacement = calculateCosts(lsschedule, i, request); 
	...
	public Solution greedyRepair(Solution solution){
		...
		while(solution.getRequests().size()!=0) { //until all requests are placed
			...
			Placement bestPlacement = Collections.max(allPlacements); 
			solution.addRequest(bestPlacement); //This method also deletes the request placed
	 ...
	 * @return
	 */
	public double calculateCosts(LSschedule lsschedule, int i, Request request) {

		ContractGroup group = lsschedule.getSchedule().getC(); 
		Schedule schedule = lsschedule.getSchedule(); 
		double[] newOvertime = lsschedule.getWeeklyOvertime(); 
		double costOfPlacement = 0;

		int[] check = schedule.getSchedule();
