...
public class FeasCheck {
	...
	public boolean isFeasible7(int[] schedule, int startDay, int endDay) {
		...
		if( endDay - startDay < 6) {
			throw new IllegalArgumentException("not more than 7 days between bounds"); 
		}
	...
	public double QuaterlyOvertime(Solution sol) {
		...
		for(ContractGroup group: sol.getNewSchedule().keySet()) {
			overtime += QuaterlyOvertime(sol.getNewSchedule().get(group).getSchedule().getSchedule(), group); 
		}
