...
public class FeasCheck {
	...
	public int ATVspread(int[] schedule, int startDay, int endDay, ContractGroup c) {
		...
		for(int k = startDay; k < endDay; k++) {
			if(schedule[k%schedule.length] == 1) {
				counter++;
			}
		}
	...
	public int checkEarlyFollowedByLate(int[] schedule, int startDate, int endDate) {
		int nOccurances = 0;
		for (int i = startDate; i < endDate; i++) {
			if (schedule[i] != 1 && schedule[i] != 2 && schedule[(i+1)%schedule.length] != 1 && schedule[(i+1)%schedule.length] != 2) {
				String dutyType1 = this.instance.getDutyTypeFromDutyNR(schedule[i]); 
				String dutyType2 = this.instance.getDutyTypeFromDutyNR(schedule[i+1]);
